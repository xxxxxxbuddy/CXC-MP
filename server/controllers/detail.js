...
module.exports = async (ctx, next) => {
  ...
  if (data.object_type==0){
    ...
    if (project.user_type == 0) {
      var information = await DB.select('individual_name').from('individual').where('individual_id', project.user_id);
      project.user_name = information[0].individual_name;
    }
    if (project.user_type == 1) {
      var information = await DB.select('company_name').from('company').where('company_id', project.user_id);
    ...
    result = project;
      ...
      if (question.user_type == 0) {
        var information = await DB.select('individual_name').from('individual').where('individual_id', question.user_id);
        question.user_name = information[0].individual_name;
      }
      if (question.user_type == 1) {
        var information = await DB.select('company_name').from('company').where('company_id', question.user_id);
