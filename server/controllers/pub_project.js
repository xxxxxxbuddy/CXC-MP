...
module.exports = async (ctx, next) => {
  ...
  var user_type=0;
  var result='发布成功';
  var x = await DB.select('*').from('company').where('company_id',data.user_id) ;
  if(x.length==0)
  {
    user_type=1;
  }
  var project_id = await DB('project').returning('project_id').insert({ user_type: user_type, user_id: data.user_id, project_title: data.project_title, project_type: data.project_type, project_finish: data.project_finish, project_budget: data.project_budget, project_time: date, project_require: data.project_require, answernum: 0, power:power.object_power,hot:100});
